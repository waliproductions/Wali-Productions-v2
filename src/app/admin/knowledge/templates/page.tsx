...
export default async function AdminTemplatesPage() {
  const templates = await knowledgeRepository.findByCategory("template");
  const today = new Date().toISOString().slice(0, 10);

  const stats = {
    total: templates.length,
    approved: templates.filter((e) => e.status === "approved").length,
    draft: templates.filter((e) => e.status === "draft").length,
    dueForReview: templates.filter((e) => e.nextReviewAt && e.nextReviewAt <= today).length,
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <AdminStatCard label="Total templates" value={stats.total} />
      ...
      <AdminCard title={`${templates.length} template${templates.length !== 1 ? "s" : ""}`} padded={false}>
        {templates.length === 0 ? (
          <div className="px-6 py-10 text-center">
            <p className="text-sm text-zinc-500">No templates documented yet.</p>
            <p className="mt-1 text-xs text-zinc-600">
              Add knowledge entries with category &ldquo;template&rdquo; to see them here.
            </p>
          </div>
        ) : (
          <AdminTable columns={COLS} rows={templates} getRowKey={(e) => e.id} />
        )}
      </AdminCard>
    </div>
  );

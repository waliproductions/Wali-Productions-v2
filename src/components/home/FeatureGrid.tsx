...
export function FeatureGrid({
  ...
  const cardClass =
    variant === "dark"
      ...
      : "rounded-xl border border-black/10 bg-white p-7 shadow-card transition-all hover:shadow-card-hover hover:border-[#4A7DB5]/25 dark:border-white/10 dark:bg-white/[0.03] dark:hover:border-white/20";
      ...
      {items.map((item, index) => {
        ...
        return (
          <li key={index} className={cardClass}>
            {icon && <div className="mb-4 text-[#1E3A5F]">{icon}</div>}
